...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
     ...
     */
    private static <T> void exchangeEntries(T[] arr, int i, int j) {
        if (i != j) {
            // Make a temporary variable for a clean swap.
            T temp = arr[i];
            arr[i] = arr[j];
            arr[j] = temp;
        }
    ...
    private static <T> void siftDown(T[] array, int top, int last,
        ...
        if (ltBounds || rtBounds) {
            ...
            if (ltBounds && rtBounds) {
                ...
                if (order.compare(array[lt], array[rt]) < 0) {
                    smallest = lt;
                } else {
                    smallest = rt;
                }
    ...
    public SortingMachine5a(Comparator<T> order) {
        // Call createNewRep with the order passed in.
