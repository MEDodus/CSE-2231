...
public class Map4<K, V> extends MapSecondary<K, V> {
    ...
    private static int mod(int a, int b) {
        ...
        // For negative numbers get the difference (left-rotation) around the "clock"
        if (a < 0 && mod != 0) {
            mod = b + mod;
        }
    ...
    private void createNewRep(int hashTableSize) {
        ...
        /*
         * Initialize size of {$this.size} (abstract rep) to size 0 since no
         * elements are in our initial rep.
         */
        this.size = 0;

        // Each Map object in the array is instantiated as a Map2
    ...
    public Map4() {
        ...
        // Create Map4 with array length == DEFAULT_HASH_TABLE_SIZE
    ...
    public Map4(int hashTableSize) {
        ...
        // Create Map4 with array length == to hashTableSize
    ...
    public final void clear() {
        this.createNewRep(this.hashTable.length);
    }
    ...
    public final void add(K key, V value) {
        ...
        // Call hashCode and mod to find bucket index
    ...
    public final Pair<K, V> removeAny() {
        ...
        // Check all buckets from index 0 to {$this.hashTable.length - 1}
        int idx = 0;
        boolean empty = true;
        while (empty && idx < this.hashTable.length) {
            // Break from loop and remove element from non-empty map
            if (this.hashTable[idx].size() > 0) {
                empty = false;
            }
            idx++;
        }
        idx--; // If it breaks from the loop, then set idx before the increment.
        this.size--;
        return this.hashTable[idx].removeAny();
    }
