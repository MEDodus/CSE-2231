 ...
 * @mathdefinitions <pre>
 * IS_TOTAL_PREORDER ( r: binary relation on T ) :
 * boolean is for all x, y, z: T ((r(x, y) or r(y, x)) and (if
 * (r(x, y) and r(y, z)) then r(x, z)))
 * </pre>
 *
 */
public interface WaitingLine<T> extends WaitingLineKernel<T> {
    ...
    /**
     * Finds {@code x} in {@code this} and returns its position.
     *
     * @param x
     *            {@code T} to find in the waitingline.
     * @return the position of x.
     * @requires x is contained in this.
     * @ensures <pre>
     * the position is the position in the waitingline.
     * </pre>
     */
    int findInLine(T x);
